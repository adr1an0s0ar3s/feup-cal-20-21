    ...
    for (Order &order : orders) {
        for (int productId : order.getProducts().getIds()) {
            ...
            if (supplierRemovedQuantity > 0) {
                supplied = true;
                order.getProducts().setQuantity(productId, orderQuantity - supplierRemovedQuantity, products);
                supplier->getStock().setQuantity(productId, supplier->getStock().getQuantity(productId) - supplierRemovedQuantity, products);
            }
        }
    }
    return supplied;
